...
def test_check_elasticsearch_ok_status(mock_elasticsearch_service):
    ...
    runner = CliRunner()
    result = runner.invoke(check_service, ['--check', 'elasticsearch', '--endpoint',
...
def test_check_kibana_warning_status(mock_kibana_service):
    ...
    result = runner.invoke(check_service, [
                           '--check', 'kibana', '--endpoint', 'https://localhost:5601', '--user',
...
def test_check_logstash_critical_status(mock_logstash_service):
    ...
    runner = CliRunner()
    result = runner.invoke(check_service, ['--check', 'logstash', '--endpoint',
...
def test_check_kibana_timeout_error(mock_kibana_service):
    ...
    runner = CliRunner()
    result = runner.invoke(check_service, [
                           '--check', 'kibana', '--endpoint', 'https://localhost:5601', '--user', 'elastic', '--password',
                           'changeme'])
    assert result.exit_code == 3
